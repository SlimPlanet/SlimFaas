            ...
            return (
              <React.Fragment key={job.Name}>
                <tr className="job-table__row">
                  ...
                  <td className="job-table__td">
                    <span className={`job-table__running ${runningCount > 0 ? 'job-table__running--active' : ''}`}>
                      {runningCount}
                    </span>
                  </td>
                  ...
                  <tr className="job-table__row job-table__row--details">
                    <td className="job-table__td" colSpan={7}>
                      <div className="job-table__details">
                        {runningCount > 0 && (
                          <div className="job-table__section">
                            ...
                            <table className="job-table__sub-table">
                              <thead><tr><th>Name</th><th>Status</th><th>Element</th><th>Queued</th><th>Started</th></tr></thead>
                              <tbody>
                        ...
                        {schedulesCount > 0 && (
                          <div className="job-table__section">
                            <h4 className="job-table__section-title">🗓️ Schedules ({schedulesCount})</h4>
                            <table className="job-table__sub-table">
                              <thead><tr><th>ID</th><th>Cron</th><th>Image</th><th>Next Execution</th></tr></thead>
                              <tbody>
                                {(job.Schedules ?? []).map((s) => (
                                  <tr key={s.Id}>
                                    <td>{s.Id}</td>
                        ...
                        {runningCount === 0 && schedulesCount === 0 && (
                          <span className="job-table__empty">No running jobs or schedules.</span>
                        )}
                      </div>
                    </td>
                  </tr>
