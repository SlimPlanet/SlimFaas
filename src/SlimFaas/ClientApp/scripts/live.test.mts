...
test('native loopback and unknown addresses stay external, mapped IPv4 resolves a pod', () => {
  const topology = model();
  ...
  assert.equal(resolveSource(topology, 'external', '::ffff:10.1.0.1'), 'pod:fibonacci1/fibonacci1-00000');
});
