...
| GET | `/status-data-stream` | SSE metadata inventory. `kind=sets/files`, `prefix`, exclusive `after`, `limit=1..500` (default 100). Emits keys, Unix-ms expiry and file sizes only. `404` for disallowed access/front/port, `400` for invalid queries, `429` for the shared SSE client limit. See [Data inventory](user-interface.md#data-inventory). |
