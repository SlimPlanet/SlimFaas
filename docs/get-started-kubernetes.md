...
SlimFaas verifies the API server certificate with the CA projected into its pod. If your API server presents a certificate that cannot be validated, `SlimFaas__KubernetesSkipTlsVerify=true` restores the unverified connection and logs a warning at startup; prefer fixing the certificate. If an ingress controller or reverse proxy must pass the original client address for Private functions, declare it in `SlimFaas__TrustedProxies__0` (IP or CIDR); otherwise `X-Forwarded-For` is ignored. See [Functions](functions.md#how-callers-are-classified).
