...
public class KubernetesResourceSignalTests
{
    ...
    [Fact]
    public void IsHealthyByDefault()
    {
        var signal = new KubernetesResourceSignal();
    ...
    [Fact]
    public async Task ReportStreamDownMarksUnhealthyAndWakesAPendingWaiter()
    {
        var signal = new KubernetesResourceSignal();
        long observed = signal.Version;
        Task<long> waiter = signal.WaitForChangeAsync(observed, TimeSpan.FromMinutes(5), CancellationToken.None);

        signal.ReportStreamDown();

        // Le waiter bloqué sur le (long) resync doit se réveiller pour réévaluer sa cadence.
        long result = await waiter.WaitAsync(TimeSpan.FromSeconds(5));
        Assert.Equal(observed + 1, result);
        Assert.False(signal.IsHealthy);
    }

    [Fact]
    public void IsHealthyAgainOnlyWhenEveryDownStreamIsBackUp()
    {
        var signal = new KubernetesResourceSignal();

        // Deux flux alimentent le même signal (ex. pods + deployments → Functions).
        signal.ReportStreamDown();
        signal.ReportStreamDown();
        Assert.False(signal.IsHealthy);

        signal.ReportStreamUp();
        Assert.False(signal.IsHealthy);
