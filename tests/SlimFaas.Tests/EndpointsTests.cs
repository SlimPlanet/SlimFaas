        ...
        new(
            new List<DeploymentInformation>
            {
                new(Replicas: 0, Deployment: "fibonacci", Namespace: "default",
                    Pods: new List<PodInformation> { new("", true, true, "", "", new List<int>() { 5000 }) },
...
public class EndpointsTests
{
    ...
    public async Task WakeFunctionEndpointAndReturnOk(string path, HttpStatusCode expectedHttpStatusCode,
    ...
    [Theory]
    [InlineData("/status-function/fibonacci", HttpStatusCode.OK, "{\"numberReady\":1,\"numberRequested\":0,\"podType\":\"Deployment\",\"visibility\":\"Public\",\"functionName\":\"fibonacci\"}")]
    [InlineData("/status-function/wrong", HttpStatusCode.NotFound, "")]
    [InlineData("/status-functions", HttpStatusCode.OK, "[{\"numberReady\":1,\"numberRequested\":0,\"podType\":\"Deployment\",\"visibility\":\"Public\",\"functionName\":\"fibonacci\"}]")]
    public async Task GetStatusFunctionEndpointAndReturnOk(string path, HttpStatusCode expectedHttpStatusCode,
        string expectedBody)
    {
        Mock<IWakeUpFunction> wakeUpFunctionMock = new();
        Mock<IJobService> jobServiceMock = new();
