...
public class SlimJobsWorkerTests
{
    ...
    public async Task ExecuteAsync_Master_OneMessageAndReplicaOk_JobCreated()
    {
        ...
        [Fact]
    public async Task ExecuteAsync_Master_ScheduledQueueItem_JobCreated()
    {
        // 1) ARRANGE ----------------------------------------------------
        _masterServiceMock.Setup(m => m.IsMaster).Returns(true);
        ...
        {
            ["fibonacci"] = new SlimfaasJob("myImage", new List<string>())
        };
        var schedulesDict = new Dictionary<string, IList<ScheduleCreateJob>>
        {
            ["fibonacci"] = new List<ScheduleCreateJob>
            {
                new ScheduleCreateJob(cron, new List<string>())
            }
        };
        var jobConfig = new SlimFaasJobConfiguration(jobsDict, schedulesDict);
        _jobConfigurationMock.Setup(c => c.Configuration).Returns(jobConfig);

        _jobServiceMock.Setup(s => s.SyncJobsAsync())
                       .ReturnsAsync(new List<Job>());   // aucun job actif

        // -- Simuler un message mis en file par le scheduler -----------
        var createJob = new CreateJob(new List<string>());
        var jiq       = new JobInQueue(createJob, "fibo-1", 1);
        var bytes     = MemoryPackSerializer.Serialize(jiq);

        _jobQueueMock.Setup(q => q.CountElementAsync("fibonacci",
                                                     It.IsAny<IList<CountType>>(),
                                                     It.IsAny<int>()))
                     .ReturnsAsync(new List<QueueData> { new("id-1", bytes) });

        _jobQueueMock.Setup(q => q.DequeueAsync("fibonacci", 1))
                     .ReturnsAsync(new List<QueueData> { new("id-1", bytes) });

        _jobQueueMock.Setup(q => q.ListCallbackAsync("fibonacci",
                                                     It.IsAny<ListQueueItemStatus>()))
                     .Returns(Task.CompletedTask);

        _jobServiceMock.Setup(s => s.CreateJobAsync("fibonacci",
                                                    It.IsAny<CreateJob>(),
                                                    It.IsAny<string>(),
                                                    It.IsAny<string>(),
                                                    It.IsAny<long>()))
        ...
        _replicasServiceMock.Setup(r => r.Deployments)
                            .Returns(new DeploymentsInformations(
        ...
        var worker = new SlimJobsWorker(
            ...
            _masterServiceMock.Object,
            _replicasServiceMock.Object,
            delay: 10      // boucle très courte pour le test
        );
        ...
        // 3) ASSERT -----------------------------------------------------
        _jobQueueMock.Verify(q => q.DequeueAsync("fibonacci", 1), Times.AtLeastOnce);
        _jobServiceMock.Verify(s => s.CreateJobAsync("fibonacci",
                                                     It.IsAny<CreateJob>(),
                                                     It.IsAny<string>(),
                                                     It.IsAny<string>(),
                                                     It.IsAny<long>()),
